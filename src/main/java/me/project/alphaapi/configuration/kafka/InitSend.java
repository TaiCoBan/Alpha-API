...
class InitSend {
    ...
    void initiateSendingMessage(ApplicationReadyEvent event) throws InterruptedException {
        ...
        Thread.sleep(5000);
        LOG.info("---------------------------------");
        kafkaSenderExample.sendCustomMessage(new ProductDTO("Lucario"), "reflectoring-user");

        Thread.sleep(5000);
        LOG.info("---------------------------------");
        messageConverterSender.sendMessageWithConverter(new GenericMessage<>(new ProductDTO("Pikachu")));
    }
}
